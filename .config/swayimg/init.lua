...
-- function to confirm, feel free to reuse, only used on delete
local function confirm(prompt)
    local handle =
        io.popen("printf 'Yes\\nNo' | wofi -c ~/.config/wofi/yes_or_no.conf --dmenu --prompt '" .. prompt .. "'")
...
swayimg.gallery.on_key("Delete", function()
    local image = swayimg.gallery.get_image()
    if confirm("Delete this image?") then
...
-- Set wallpaper with awww and update colors with pywal
swayimg.viewer.on_key("Ctrl+w", function()
    local image = swayimg.viewer.get_image()
    os.execute(
        string.format(
            "awww img '%s' --transition-type wipe --transition-duration 1 & wal -i '%s' && $HOME/.local/scripts/helpers/refresh_windowborder",
            image.path,
            image.path
        )
    )
